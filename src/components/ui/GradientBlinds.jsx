...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // Dynamic floating angle variation over time
    float dynamicAngle = uAngle + sin(iTime * 0.35) * 0.08;
    vec2 pr = rotate2D(p, dynamicAngle);
    pr.x /= aspect;
    vec2 uv = pr * 0.5 + 0.5;

    vec2 uvMod = uv;
    // Fluid continuous time-based wave distortion
    float timeShift = iTime * 0.45;
    float a = uvMod.y * 6.0 + timeShift;
    float b = uvMod.x * 6.0 - timeShift * 0.7;
    float w = 0.015 * max(uDistort, 0.6);
    uvMod.x += sin(a) * w;
    uvMod.y += cos(b) * w;

    // Dynamic traveling color flow
    float t = uvMod.x + sin(iTime * 0.2) * 0.08;
    if (uMirror > 0.5) {
    ...
    // Dynamic moving blind stripes phase
    float blindCount = max(uBlindCount, 1.0);
    float stripePhase = (uvMod.x + sin(iTime * 0.25) * 0.04) * blindCount;

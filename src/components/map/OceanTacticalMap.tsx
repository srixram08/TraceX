...
export const OceanTacticalMap: React.FC<OceanTacticalMapProps> = ({
  ...
  const [baseMapStyle, setBaseMapStyle] = useState<'voyager' | 'dark' | 'satellite'>('voyager');
  ...
  return (
    <div className="relative w-full h-full min-h-[520px] bg-slate-100 rounded-3xl overflow-hidden border border-slate-200 shadow-xl flex flex-col">
      
      {/* Top Left Coordinates Card */}
      <div className="absolute top-4 left-4 z-[400] flex items-center gap-2">
        <div className="bg-white/95 backdrop-blur-md px-4 py-2 rounded-2xl border border-slate-200 shadow-lg flex items-center gap-2.5 text-xs font-mono text-slate-800">
          <Radio className="w-4 h-4 text-purple-600 animate-pulse" />
          <span className="font-extrabold text-slate-900 font-display">OCEAN TACTICAL MAP</span>
          <span className="text-slate-300">|</span>
          <span className="font-bold text-purple-700">{incident.coordinates.lat.toFixed(4)}°N, {incident.coordinates.lng.toFixed(4)}°E</span>
        </div>

        <button
          onClick={handleResetZoom}
          className="bg-white/95 backdrop-blur-md p-2.5 rounded-2xl border border-slate-200 shadow-lg text-slate-700 hover:text-purple-600 hover:border-purple-300 transition-all"
          title="Reset Zoom"
        >
        ...
        <div className="bg-white/95 backdrop-blur-md p-1 rounded-2xl border border-slate-200 shadow-lg flex items-center gap-1 text-[11px] font-mono">
          <button
            onClick={() => setBaseMapStyle('voyager')}
            className={`px-2.5 py-1 rounded-xl transition-all font-bold ${
              baseMapStyle === 'voyager' ? 'bg-purple-600 text-white shadow-sm' : 'text-slate-600 hover:text-purple-600'
            }`}
          >
            Light
          </button>
          <button
            ...
            className={`px-2.5 py-1 rounded-xl transition-all font-bold ${
              baseMapStyle === 'satellite' ? 'bg-purple-600 text-white shadow-sm' : 'text-slate-600 hover:text-purple-600'
            }`}
          >
            Satellite
          </button>
          <button
            ...
            className={`px-2.5 py-1 rounded-xl transition-all font-bold ${
              baseMapStyle === 'dark' ? 'bg-purple-600 text-white shadow-sm' : 'text-slate-600 hover:text-purple-600'
            }`}
          >
            Navy
          </button>
      ...
      {/* Top Right Layer Palette Card (Crisp White Card with Dark Readable Text!) */}
      <div className="absolute top-4 right-4 z-[400] bg-white/95 backdrop-blur-md p-4 rounded-3xl border border-slate-200 shadow-xl max-w-xs space-y-2.5">
        <div className="flex items-center justify-between pb-2 border-b border-slate-100 font-mono text-xs font-bold uppercase tracking-wider text-slate-800">
        ...
        <div className="space-y-1.5 text-xs font-mono font-medium">
          <label className="flex items-center justify-between cursor-pointer text-slate-800 hover:text-purple-700 p-1 hover:bg-slate-50 rounded-lg">
        ...
        <div className="pt-2 border-t border-slate-100 text-[11px] font-mono text-slate-600 space-y-1">
          <div className="flex items-center justify-between">
            ...
            <span className="text-slate-900 font-bold">{incident.windSpeedKnots} kn @ {incident.windDirectionDeg}°</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="flex items-center gap-1.5 text-slate-500">
              <Compass className="w-3.5 h-3.5 text-emerald-600" /> Current:
            </span>
            <span className="text-slate-900 font-bold">{incident.currentKnots} kn @ {incident.currentDirectionDeg}°</span>
          </div>
        </div>
      </div>
      ...
      {/* Bottom Bar Info */}
      <div className="absolute bottom-4 left-4 z-[400] bg-white/95 backdrop-blur-md px-4 py-2 rounded-2xl border border-slate-200 shadow-lg text-xs font-mono text-slate-700 flex items-center gap-3">
        <span className="flex items-center gap-1.5 font-bold text-purple-700">
          <Shield className="w-4 h-4 text-purple-600" /> SIH26143 / NTRO TACTICAL GRID
        </span>
        <span className="text-slate-300">•</span>
        <span>Reconstructed Time: <strong className="text-slate-900">T-{rewindHours.toFixed(1)}h</strong></span>
      </div>
